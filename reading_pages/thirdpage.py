...
class MainWindow(tk.Tk):
    def __init__(self):
        ...
        # Load text from file
        self.load_text_from_file("reading_pages/reading_passage.txt")

        self.create_text_boxes("reading_pages/questions.txt")
    ...
    def load_text_from_file(self, file_path):
        try:
            with open(file_path, "r") as file:
                ...
                canvas.place(x=10, y=self.winfo_screenheight() * 0.05, relwidth=0.35, relheight=0.9)
    ...
    def create_text_boxes(self, file_path):
        tb_height = int(self.winfo_screenheight() * 0.9 / 2 * 0.9)
        num_columns = 1
        left_margin_percentage = 0.4  # 30% space from the left side of the screen
        ...
        for j in range(num_columns):
            ...
            top_label = tk.Label(self, text=column_labels[j], bg="#333333", fg="white", font=("Arial", 12),
                                highlightthickness=2, highlightbackground="#FFFFFF", highlightcolor="#FFFFFF")
            top_label.place(x=tb_x_start + j * tb_width, y=top_label_y, width=tb_width, height=tb_height * 0.1)

            # Create text boxes below the labels
            text_box = tk.Text(self, bg="#333333", fg="white", font=("Arial", 12), wrap=tk.WORD)
            text_box.place(x=tb_x_start + j * tb_width, y=top_label_y + tb_height * 0.1, width=tb_width, height=tb_height)

            # Load text from file into the text box
            try:
                with open(file_path, "r") as file:
                    text = file.read()
    ...
    def create_cages(self):
        cage_height = int(self.winfo_screenheight() * 0.9/10 * 0.9)
        num_cages = 3
        left_margin_percentage = 0.4  # 30% space from the left side of the screen

        # Calculate the width of the cages after leaving space on the left
        cage_width = int((self.winfo_screenwidth() * (1 - left_margin_percentage)) / num_cages)
        cage_x_start = int(self.winfo_screenwidth() * left_margin_percentage)  # Starting X position

        top_margin_percentage = 0.5  # 0.05% space from the top of the screen
