...
class FirestoreApp(QWidget):
    ...
    def initUI(self):
        ...
        if os.path.exists(paragraph_file_path):
            ...
            paragraph_label.setText("No paragraph available for the selected paper.")

        # Difficulty level input
        difficulty_layout = QHBoxLayout()  # Create QHBoxLayout for difficulty level input
        difficulty_label = QLabel('Enter you Name', self)
        ...
        layout.addWidget(self.nextButton, alignment=Qt.AlignRight | Qt.AlignBottom) 
    ...
    def openNextPage(self):
        ...
        actions = {
            "Reading": lambda: subprocess.Popen(["python", "reading_pages/thirdpage.py"]),
            "Listening": lambda: subprocess.Popen(["python", "reading_pages/thirdpage.py"]),
        }
        ...
        if action:
            ...
            print("No action defined for the selected paper.")
